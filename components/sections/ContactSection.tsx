...
export default function ContactSection() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Use exact template variable names from EmailJS
    const emailData = {
      name: formData.name,
    ...
    emailjs.send("service_dzi7ay5", "template_f8a8bgq", emailData, "kTR8adugPvmZ6GaT-")
      .then((result) => {
      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <form onSubmit={handleSubmit} className="bg-gray-700 lg:p-6 p-4 rounded-lg shadow-md">
            ...
            <button
              type="submit"
              className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-2 px-6 rounded transition duration-300 w-full cursor-pointer"
            >
              Send Message
            </button>
          </form>
          ...
          <div className="bg-gray-700 lg:p-6 p-4 rounded-lg shadow-md h-full">
            ...
            <div className="space-y-4">
              {[
                { Icon: MapPin, label: "Our Location", text: "Mission Road Surat, Gujarat" },
            ...
            <div className="lg:mt-8 mt-5">
              ...
              <div className="flex space-x-4">
                {socialLinks.map((link, index) => (
                  <a key={index} href="#" className={`w-10 h-10 ${link.bg} rounded-full flex items-center justify-center ${link.hover} transition cursor-pointer`}>
                    <link.icon size={18} />
                  </a>
                ))}
              </div>
